...
def pytest_runtest_teardown(item, nextitem):
    ...
    if (
        ...
        setattr(item.session._setupstate, "stack_backup", item.session._setupstate.stack.copy())
...
def pytest_runtest_protocol(item, nextitem):
    ...
    while need_to_run:
        ...
        if should_not_rerun and hasattr(item.session._setupstate, "stack_backup"):
            # trigger session teardown from backup since rerun was canceled due to teardown condition
            item.session._setupstate.stack = getattr(item.session._setupstate, "stack_backup")
